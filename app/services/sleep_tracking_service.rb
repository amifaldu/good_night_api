class SleepTrackingService
  ...
  def clock_in
    ...
    if last_sleep_record.present?
      add_error(I18n.t('api.clock_in.clock_out_remaining'))
      return @response
    end
    ...
    if sleep_record.persisted?
      ...
      set_success(I18n.t('api.clock_in.success'), saved_records)
    else
      add_error(I18n.t('api.clock_in.erorr'))
    end
    return @response
  end
  ...
  def clock_out
    ...
    if last_sleep_record.present? && last_sleep_record.sleep_end.nil?
      ...
      set_success(I18n.t('api.clock_out.success'), update_record)
    else
      add_error(I18n.t('api.clock_out.erorr'))
    end
    return @response
  end
